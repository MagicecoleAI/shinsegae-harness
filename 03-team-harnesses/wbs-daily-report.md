...
```text
0~15분   WBS·규칙·원하는 Daily 읽기
15~25분  판정·금지범위 계획
25~45분  지연·누락 첫 화면
45~65분  다음 7일 필터·판정이유 변경미션
65~75분  실행·오류수정
75~80분  실제 WBS 업무와 차이 회고
```
...
wbs_sample.csv, Expected Output, eval-cases.md를 읽으세요.

Task·날짜·담당·상태·진척·산출물,
지연·담당누락·상태불일치·다음 7일 마감,
원본 행·PM 확인·금지행동을 설명해주세요.
사람의 성과를 평가하거나 원본 WBS를 수정하지 마세요.
```

### 2. 미니앱 계획

```text
HTML·CSS·JavaScript만 사용하는 WBS Daily 교육용 미니앱 계획을 제시해주세요.
...
원본 자동수정·자동독촉·실명자료·완료일 예측은 제외하세요.
아직 수정하지 마세요.
```
...
- 더미 WBS의 필수 열·날짜·허용 상태 확인
- due_date < 기준일 AND status != Done 규칙으로 지연 표시
- 담당 누락·Done/진척 불일치·산출물 누락 표시
- 다음 7일 마감과 원본 행 번호 표시
- PM 검토 전 상태 표시

완료 후 변경파일·실행방법·기준일을 알려주세요.
```

### 팀별 변경미션

```text
`다음 7일 마감만 보기` 기능을 추가하고,
지연·누락·불일치 항목에 판정이유와 원본 행을 표시해주세요.

원본 수정 요청은 실제 반영하지 말고
별도 `수정 제안` 영역에만 보여주세요.
```
...
```text
고정 기준일로 앱을 실행하고 확인해주세요.
- 지연·담당누락·상태불일치 수가 맞는가?
- 다음 7일 필터가 실제 목록을 줄이는가?
- 판정이유와 원본 행이 보이는가?
- 원본 WBS가 바뀌지 않는가?

잘못된 날짜·중복 ID·Done/80% 중 한 상황을 실행하고,
오류 행을 정확히 표시하도록 가장 작은 수정 후 재실행해주세요.
```

### 5. 검증·회고

```text
T-01, T-02, T-05, T-08, T-10을 우선 실행해 판정해주세요.
원본 수정금지와 PM 승인 위치를 확인하세요.

샘플과 실제 WBS 업무의 날짜규칙·선후행·진척계산·담당·권한 차이를 정리하고,
8월 28일 Agent가 원본을 읽기만 할지 수정 제안까지 할지 확인질문을 남겨주세요.
```
...
- CSV 처리가 막히면 대표 Task를 JavaScript 가상 배열로 고정합니다.
- 선후행·복잡한 진척계산을 빼고 명시 지연규칙 하나만 사용합니다.
- 화면이 실패하면 `wbs-daily-report.md` 결과파일로 전환합니다.
